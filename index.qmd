...
df_fig1 <- summary_wide %>%
    filter(iso3 == ctry, labels_index %in% lev_2, time >= 2000) %>%
    drop_na() %>%
    pivot_wider(
        names_from = index,
        values_from = value
...
fig3 <- fig3 %>%
    layout(
        title = list(text = "Figure 3: Share of workers in most productive (tradable) sectors, TL2 regions", x = 0),
        margin = list(
            l = 50, r = 50,
            b = 50, t = 120,
            pad = 4
...
fig3 <- fig3 %>%
    layout(
        title = list(text = "Figure 3: Share of workers in most productive (tradable) sectors, TL2 regions", x = 0, font = list(color = "black"))
    )
