```{r gbr_ctry}
ctry <- "GBR"
ctry2 <- "United Kingdom"
```

# United Kingdom

```{r gbr_pkgs, warning=FALSE}
...
paragraph_theil <- glue("{ ctry3 } experienced { theil_chg_txt } in the Theil index of GDP per capita over 2000-2020. Inequality reached its maximum in { theil_max }. The figures were normalized, with the values in the year {as.integer(min_y)} set to 1.

Polarisation, as measured by the Top 20%/Mean ratio was { paste(abs(polarization_pct), polarization_txt) } in { min(polarization_yrs) } compared to { max(polarization_yrs) }. Bottom divergence, as measured by the Bottom 20%/Mean ratio was { paste(abs(polarization_2_pct), polarization_2_txt) } in the same period.")
```
...
fig3_title <- if (any(ctry %in% c("USA","TUR","NOR"))) {
