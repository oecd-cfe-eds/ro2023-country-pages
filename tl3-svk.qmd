```{r svk_ctry}
ctry <- "SVK"
ctry2 <- "Slovak Republic"
```

# Slovak Republic

```{r svk_pkgs, warning=FALSE}
...
In `r ctry3`, labour productivity in higher productivity regions grew by `r hpgrew`% between `r min(yrs)` and `r max(yrs)`, `r abs(hpdiff)` percentage points `r hpmore` compared to lower productivity regions.
...
tghpmore <- ifelse(tghpgrew > 0, "grew", "declined")

# tglpdiff <- tglpgrew - tglpgrew
tglpmore <- ifelse(tglpgrew > 0, "grew", "declined")
...
tshpmore <- ifelse(tshpgrew > 0, "grew", "declined")

# tslpdiff <- tslpgrew - tslpgrew
tslpmore <- ifelse(tslpgrew > 0, "grew", "declined")
...
dp21 <- dp21 %>%
  mutate(
    ...
    category2 = str_replace_all(category2, "LP", "Lower productivity"),
    category2 = str_replace_all(category2, "HP", "Higher productivity")
  )
...
fig4_title <- if (any(ctry %in% c("USA","TUR","NOR", "CHE"))) {
...
#   rename(
#     series = category2_2,
#     hp_min_y = higher_productivity_minyr,
#     hp_max_y = higher_productivity_maxyr,
#     lp_min_y = lower_productivity_minyr,
#     lp_max_y = lower_productivity_maxyr
#   ) %>%
#   ggplot() +
...
dp21_2 <- dp21 %>%
  ...
  pivot_wider(names_from = time, values_from = value_country)

fig4_2 <- dp21_2 %>% 
  ggplot() +
  geom_bar(aes(x = type, y = `2001`, fill = type),
           stat = "identity", position=position_dodge(), width=1) +
  geom_point(aes(x = type, y = `2019`, color = type),
             size = 5,
             shape = 18) +
  scale_fill_manual(values = clrs4[1:2], 
                    # labels = c("Lower productivity 2001", "Higher productivity 2001"), 
                    name = "2001") + 
  scale_color_manual(values = c("#6bc5f2", "#c8f075"), 
                     # labels = c("Lower productivity 2019", "Higher productivity 2019"),
                     name = "2019") +
  facet_wrap(~category1, strip.position = "bottom") +
    labs(x = "", y = "Employment share (%)", color = "",
         title = fig4_title) +
  theme_minimal() +
  theme(plot.title = element_text(size = 13, hjust = 0, margin=margin(0,0,10,0)),
...
p <- p %>% 
  layout(
    title = list(text = fig4_title, x = 0), 
...
In `r ctry3`, the share of employment in the tradable goods sector `r tghpmore` by `r abs(tghpgrew)` percentage points in higher productivity regions between `r min(yrs)` and `r max(yrs)`, and it `r tglpmore` by `r abs(tglpgrew)` percentage points in lower productivity ones. At the same time the share of employment in tradable services `r tshpmore` by `r abs(tshpgrew)` percentage points in higher productivity regions between `r min(yrs)` and `r max(yrs)`, and it `r tslpmore` by `r abs(tslpgrew)` percentage points in lower productivity ones.
...
<div class="oecdnote">
**Note**: The employment share for higher productivity (lower productivity) regions in a given sector is defined as total employment across higher productivity (lower productivity) regions in a sector/year divided by total employment across higher productivity (lower productivity) regions in a year. Regions are at the TL2 level for Australia, Canada, Chile, Ireland, Mexico, Norway, Switzerland, Turkey and the United States. A region is defined as having higher productivity if its productivity was equal or above the country median for at least two years out of the first four years of each region’ observation period, and as having lower productivity otherwise. The tradable goods sector includes Mining and quarrying (B), Manufacturing (C), Electricity, gas, steam and air conditioning supply (D) and Water supply; sewerage; waste management and remediation activities (E) NACE macro sectors. Tradable services include Information and communication (J), Financial and insurance activities (K), Real estate activities (L), Professional, scientific and technical activities (M), Administrative and support service activities (N). The starting year is the first available year of data for each country.<br>
**Source**: OECD Regional Database (2022).
</div>
